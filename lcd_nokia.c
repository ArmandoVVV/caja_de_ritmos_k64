...
static const uint8_t string_manual[]= "1)Manual";
static const uint8_t string_sequence[]= "2)Sequence";
static const uint8_t string_manual_mode[]= "Manual Mode";
static const uint8_t string_press_any[]= "Press Any";
static const uint8_t string_key[]= "Key";
static const uint8_t string_sequence_mode[] = "Sequence:";
static const uint8_t string_stopped[] = "Stopped";
static const uint8_t string_playing[] = "Playing";

void LCD_write_menu(void){
	LCD_nokia_clear();
	LCD_nokia_goto_xy(1,2);
	LCD_nokia_send_string(&string_manual[0]);
	LCD_nokia_goto_xy(1,3);
	LCD_nokia_send_string(string_sequence);
}

void LCD_write_playing(void){
	LCD_nokia_goto_xy(10,5);
	LCD_nokia_send_string(string_playing);
}

void LCD_write_stopped(void){
	LCD_nokia_goto_xy(10,5);
	LCD_nokia_send_string(string_stopped);
}

void LCD_write_manual_mode(void){
	LCD_nokia_clear();
	LCD_nokia_goto_xy(1,1);
	LCD_nokia_send_string(&string_manual_mode[0]);
	LCD_nokia_goto_xy(10,2);
	LCD_nokia_send_string(string_press_any);
	LCD_nokia_goto_xy(25,3);
	LCD_nokia_send_string(string_key);
}

void LCD_write_sequence_mode(void){
	LCD_nokia_clear();
	LCD_nokia_goto_xy(1,0);
	LCD_nokia_send_string(&string_sequence_mode[0]);
	LCD_nokia_goto_xy(10,5);
	LCD_nokia_send_string(string_stopped);
}

void LCD_welcome_image(void){
	LCD_nokia_bitmap(get_welcome_image());
}

void LCD_error_image(void){
	LCD_nokia_bitmap(get_error_image());
}
...
void LCD_nokia_bitmap(const uint8_t bitmap[]){
   ...
   dspi_transfer_t transfer_config = {
			.dataSize = 504,
			.rxData = NULL,
			.txData = (uint8_t*)bitmap,
			.configFlags = kDSPI_MasterCtar0
   };
   DSPI_MasterTransferBlocking(SPI0, &transfer_config);
}
...
void LCD_nokia_send_string(const uint8_t characters []) {
  while (*characters)
